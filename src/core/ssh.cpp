...
ssh_session Ssh::startSession(connectionInfo ci) {
    ...
    session = ssh_new();
    if (session == NULL) {
        throwChronicleException(202,"Ssh::startSession",ssh_get_error(session));
    }
    ...
    if (rc != SSH_OK)
    {
      throwChronicleException(203,"Ssh::startSession",ssh_get_error(session));
    }

    std::string known_host_msg = verifyKnownHost(session);
    if (known_host_msg.size() > 0) {
        endSession(session);
        throwChronicleException(202,"Ssh::startSession",known_host_msg);
    }
...
int Ssh::executeCommand(const char *command, ssh_session session, std::vector<std::string> &output_lines) {
    ...
    if (!ssh_is_connected(session)) {
        endSession(session);
        throwChronicleException(200, "Ssh::executeCommand", "SSH session died before executing command: `" + std::string(command) + "`.");
    }
   
    channel = ssh_channel_new(session);
    if (channel == NULL)
        throwChronicleException(202,"Ssh::executeCommand",ssh_get_error(session));
