...
int connectionInfo::getDeviceId(int vendor_id, std::string device_name) {
    chronicleAssert(vendor_id > 0, 109, "No vendor ID found for " + device_name);
    if (device_name == "ENCS 5100") {
        return 1;
    } else if (device_name == "C1700") {
        return 2;
    } else if (device_name == "C7200") {
        return 2;
    } else if (device_name == "Catalyst-8000V") {
        return 3;
    } else {
        return 0;
    }
...
connectionInfo getConnectionInfo(std::string section) {
    if (!chronicleConfig.HasSection(section)) throwChronicleException(112, "(" + section + ")");
    connectionInfo ci;
    int vendor_id = ci.getVendorId(chronicleConfig.Get(section, "Vendor", "UNKNOWN"));
        chronicleAssert(vendor_id > 0, 101, "Vendor ID not found for " + section);
    int device_id = ci.getDeviceId(vendor_id, chronicleConfig.Get(section, "DeviceName", "UNKNOWN"));
        chronicleAssert(device_id > 0, 101, "Device ID not found for " + section);
    int verbosity = chronicleConfig.GetInteger(section, "SshVerbosity", CHRONICLE_CONFIG_DEFUALT_VERBOSITY);
        chronicleAssert(4 >= verbosity && verbosity >= 0, 101, "SshVerbosity can only be between 0-4");
    ci.vendor =                 vendor_id;
    ci.device =                 device_id;
