...
extern "C" deviceOperations* createDeviceOperations(int device_id, int vendor_id) {
    ...
    if (vendor_id != JUNIPER_ID) {
        THROW_CHRONICLE_EXCEPTION(301, "Vendor ID does not match Juniper. (" + std::to_string(vendor_id) + " instead of " + std::to_string(JUNIPER_ID) + ")");
        return nullptr;
    };
    
    if (device_id != VSRX) {
        THROW_CHRONICLE_EXCEPTION(301, "Device ID does not match CATALYST_8000V. (" + std::to_string(device_id) + " instead of " + std::to_string(VSRX) + ")");
        return nullptr;
    };
